...
namespace RayTracer
{
    public class Perlin
    {
        ...
        {
            int xi = ((int)(Math.Floor(x) % 256) + 256) % 256;		// Calculate the "unit cube" that the point asked will be located in
            int yi = ((int)(Math.Floor(y) % 256) + 256) % 256;		// The left bound is ( |_x_|,|_y_|,|_z_| ) and the right bound is that
            int zi = ((int)(Math.Floor(z) % 256) + 256) % 256;		// plus 1.  Next we calculate the location (from 0.0 to 1.0) in that cube.
