...
namespace RayTracer.Shapes
{
    public class Cone: Shape
    {
        ...
        {
            return obj is Cylinder other && base.Equals(other) && Utilities.AlmostEqual(Radius, other.Radius) && Utilities.AlmostEqual(Minimum, other.Minimum) && Utilities.AlmostEqual(Maximum, other.Maximum);
        }

        public override int GetHashCode()
        {
            var closedHash = Closed ? 1 : -1;
        ...
        public override string ToString()
        {
            ...
            return $"[{baseStr[..materialIndex]}\nRadius:{Radius},Min:{Minimum},Max:{Maximum},Closed:{Closed}{baseStr[materialIndex..]}]";
        }
        ...
        public override Vector LocalNormalAt(Point objectPoint, Intersection intersect = null)
        {
            ...
            return y switch
            {
                < 1 when objectPoint.Y >= Maximum - Utilities.EPSILON => new Vector(0, 1, 0),
                < 1 when objectPoint.Y <= Minimum + Utilities.EPSILON => new Vector(0, -1, 0),
                _ => new Vector(objectPoint.X, y, objectPoint.Z)
            };
        }
