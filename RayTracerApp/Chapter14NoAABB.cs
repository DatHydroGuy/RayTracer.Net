...
{
    public class Chapter14NoAABB
    {
        public Chapter14NoAABB()
        {
            var floor = new Plane();
            ...
            int NUM = 6;
            Shape[] hs = new Shape[NUM * 2];
            for (int i = 0; i < NUM; i++)
            {
                hs[i] = HexCorner(i);
                hs[i + NUM] = HexEdge(i);
                hs[i].Transform = Transformations.Translation(0, 0.5, 0) * Transformations.Scaling(2, 2, 2) * hs[i].Transform;
                hs[i + NUM].Transform = Transformations.Translation(0, 0.5, 0) * Transformations.Scaling(2, 2, 2) * hs[i + NUM].Transform;
            }

            var world = new World();
            world.AddShapesToWorld(new Shape[] {floor, sky});
            world.AddShapesToWorld(hs);
            ...
            canvas.WriteImageFile("I:\\Programming\\DotNetCore\\RayTracer\\RayTracer\\Images\\chapter14NoAABB");
            Console.WriteLine("Chapter14NoAABB Complete");
        }

        public Shape HexCorner(int i)
        {
            var glass = new Sphere();
            glass.Transform = Transformations.RotationY(i * Math.PI / 3.0) * Transformations.Translation(0, 0, -1) * Transformations.Scaling(0.25, 0.25, 0.25);
