...
{
    public class BlendedPatternTest
    {
        public BlendedPatternTest()
        {
            var floor = new Plane();
            var floorPattern1 = new StripePattern(new Colour(1, 0, 0), new Colour(1, 1, 1));
            floorPattern1.Transform = Transformations.Scaling(0.5, 0.5, 0.5);
            var floorPattern2 = new StripePattern(new Colour(0, 0, 1), new Colour(1, 1, 1));
            floorPattern2.Transform = Transformations.Scaling(0.5, 0.5, 0.5) * Transformations.RotationY(Math.PI / 2.0);
            var floorPattern = new BlendedPattern(floorPattern1, floorPattern2);
            floorPattern.Transform = Transformations.Scaling(0.5, 0.5, 0.5) * Transformations.RotationY(Math.PI / 5.0);
            ...
            canvas.WriteImageFile("I:\\Programming\\DotNetCore\\RayTracer\\RayTracer\\Images\\BlendedPatternTest");
            Console.WriteLine("BlendedPatternTest Complete");
        }
    }
}
